...
//  if( iMouse.z>0.0 ) m = iMouse.xy/iResolution.xy;
  ...
  // raymarch
    ...
    // lights
    vec3 brdf = vec3(0.0);
    brdf += 2.0*dif*vec3(1.00,1.00,1.00)*pow(vec3(sha),vec3(1.0,1.2,1.5));
    brdf += 1.0*amb*vec3(0.30,0.30,0.30)*occ;
    brdf += 2.0*bou*vec3(0.40,0.40,0.40)*mate2.y;
    brdf += 4.0*bac*vec3(0.40,0.30,0.25)*occ;
    brdf += 1.0*fre*vec3(1.00,1.00,1.00)*2.0*mate.w*(0.5+0.5*dif*sha)*occ;
    brdf += 1.0*spe*vec3(1.0)*occ*mate.w*dif*sha;

    // surface-light interacion
    col = mate.xyz* brdf + vec3(0.3)*mate.w*pow(spe,2.0)*sha;
  }
