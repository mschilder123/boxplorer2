// From https://www.shadertoy.com/view/4sl3Dj
...
float g_time = time;  //iChannelTime[1];
...
vec4 round2( vec2 p ) 
{
    return vec4(roundf(p.x), roundf(p.y));
}
...
void animateGlobals()
{
    ...
    g_bassBeat = g_audioResponse * smoothstep(0.93, 1.01, 1.0 - texture2D( iChannel1, vec2( 0.01, time * .01) ).r);
    ...
    for (int i = 0; i < NUM_AUDIO_SAMPLE_TRIPLES; i += 1) {
        ...
        g_audioFreqs[i] = smoothstep(vec3(0.2), vec3(0.9),
                                     vec3(1.-texture2D( iChannel1, vec2(0.05 + audioOffsets.x, 0.01)).b,
                                          1.-texture2D( iChannel1, vec2(0.05 + audioOffsets.y, 0.01)).b,
                                          1.-texture2D( iChannel1, vec2(0.05 + audioOffsets.z, 0.01)).g));
    }
...
#if 1
    if (!setup_ray(eye, dir, g_camOrigin, rayDir)) {  // boxplorify view
      return;
    }
#endif
