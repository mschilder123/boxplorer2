...
void main(void)
{
  ...
  for( int k=0; k<2; k++ )
  {
    ...
    if( tmat.y>-0.5 )
    {
        ...
        vec3 brdf = vec3(0.0);
        brdf  = 2.50*dif*vec3(1.0,0.85,0.55)*(0.6+0.4*occ)*pow( vec3(sha), vec3(1.0,1.2,1.4) );
        brdf += 0.85*wbg*wbw*vec3(1.2,0.6,0.3)*(0.5+0.5*clamp(0.5-0.5*nor.y,0.0,1.0))*pow(1.0-smoothstep(0.0,3.5,3.8+pos.y),2.0)*(0.2+0.8*occ);
        brdf += 0.025*wbg*amb*vec3(0.75,0.85,0.9)*(0.1+0.9*occ);
        brdf += (1.0-mate.xyz)*0.15*occ*vec3(1.0,0.5,0.1)*clamp(0.5+0.5*nor.x,0.0,1.0)*pow(clamp(0.5*(pos.x-1.5),0.0,1.0),2.0);

        if( tmat.y>6.5 ) brdf += sha*occ*vec3(5.0)*pow( clamp(1.0+dot(rd,nor),0.0,1.0), 3.0 );
        
        col += atten*mate.xyz*brdf;
