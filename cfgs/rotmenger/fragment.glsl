...
#define Angle par[3].x// {min=-1 max=2 step=.001}
...
float de_menger(vec3 z0) {
    ...
    for (i=0;i<iters;i++){
        ...
        z0.x=z0.x*scale-CX*(scale-CX);//CX=CY=CZ was 1.0
        z0.y=z0.y*scale-CY*(scale-CY);
        z0.z=z0.z*scale;
        if(z0.z>0.5*(scale-CZ)) z0.z-=(scale-CZ);
        z0.xyz *= RotationMatrix;
    }
    float menger= r*pow(scale,1.0-float(i));
    return max(menger,-menger);  // also known as abs()?
}
