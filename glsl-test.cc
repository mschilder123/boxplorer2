...
void sanitizeParameters(void) {
  ...
  if (fov_y <= 0)
    fov_y = atan(tan(fov_x * PI / 180 / 2) * XRES / YRES) / PI * 180 * 2;
  ...
  if ((f = fopen(configFile, "r")) != 0) {
    ...
    while (fscanf(f, " %s", s) == 1) { // read word
      ...
      if (!strcmp(s, "position")) {
        v = fscanf(f, " %f %f %f", &pos.x, &pos.y, &pos.z);
        continue;
      }
      if (!strcmp(s, "direction")) {
        v = fscanf(f, " %f %f %f", &ahead.x, &ahead.y, &ahead.z);
        continue;
      }
      if (!strcmp(s, "upDirection")) {
        v = fscanf(f, " %f %f %f", &up.x, &up.y, &up.z);
        continue;
      }
...
int vertex_main(int argc, char *argv[]) {
  ...
  mat4 proj(right.x, up.x, ahead.x, 0, right.y, up.y, ahead.y, 0, right.z, up.z,
            ahead.z, 0, 0, 0, 0, 1);
  xres = XRES;
