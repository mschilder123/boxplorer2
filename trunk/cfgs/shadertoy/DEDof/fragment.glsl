// DE DOF by eiffie (https://www.shadertoy.com/view/MsBGRh)
...
#define shadowCone par[2].x //{min=.001 max=1 step=.001}
...
void main() {
  ...
  if (!setup_ray(eye, dir, ro, rd)) {  // boxplorify view
	...
	for(int i=1;i<max_steps;i++){//march loop
		if(col.w>0.9 || t>10.0)break;//bail if we hit a surface or go out of bounds
