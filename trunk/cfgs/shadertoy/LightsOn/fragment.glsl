...
vec3 illuminatePointLight(PointLight light, Output o) {
	...
	float s = 0.0;
	if(d > 0.0) {
		vec3 eyeV = normalize(eyeP - o.origin);
...
vec3 illumiate(Output o) {
	...
	float dis = length(eyeP - o.origin);
...
void main() {
	...
	eyeP = vec3(cos(iGlobalTime * 0.1 + xo) * 5.0, yo, sin(iGlobalTime * 0.1 + xo) * 5.0);
	
	vec3 lookat = vec3(0.0, 0.0, 0.0);

  if (!setup_ray(eye, dir, eyeP, lookat)) return;
	
	//Ray ray = getPrimaryRay(eyeP, lookat);
  Ray ray;
  ray.origin = eyeP;
