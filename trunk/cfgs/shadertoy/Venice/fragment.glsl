...
float noise( in vec2 x ) {
  ...
  return -1.0 + 2.0*texture2D( iChannel0, (uv+0.5)/512.0, -100.0 ).x;
}
...
vec4 getBuildingParams( in float buildingindex ) {
  ...
  return vec4(
    ...
    h.z<0.5?1.:0.,         // sidewalk
    abs(h.z-0.4)<0.25?1.:0.    // balcony
  );
}
...
vec2 mapTerrain( in vec3 p ) {  
  ...
  if( buildingParams.w > 0. ) {
    res = opU( res, vec2( baseBalcony( vec3( pos.x, pos.y, zcenter), buildingParams.x+2.5  ), 9. ) );
  }
#endif
  
#ifdef SHOW_GALLERY
  if( buildingParams.z > 0. ) {
    pos.x += -8.75-GALLERYINSET;    
    res =  opU( res, vec2( baseGallery( vec3( pos.x, pos.y, zcenter) ), 5. ) );
  }
#endif  
