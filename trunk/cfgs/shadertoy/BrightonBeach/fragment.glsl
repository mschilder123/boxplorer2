...
vec3 ApplyFog( in vec3  rgb, in float dis, in vec3 direction)
{
  float fogAmount = clamp(dis* 0.000022, 0.0, 1.0);
  return mix( rgb, GetSky(direction), fogAmount );
}
...
//--------------------------------------------------------------------------
vec3 TerrainColour(vec3 pos, vec3 direction,  vec3 normal, float dis, float type)
{
  vec3 mat;
  dis *= dis;
  if (type >= 1.0)
  {
    // Sea...
    normal = normalize(reflect(direction, normal));
    mat = GetSky(normal);
    ...
    mat = ApplyFog(mat, dis, direction);
    return mat;
  }else
  {
    // Land...
    ...
    DoLighting(mat, pos, normal,direction, dis);
...
vec3 PostEffects(vec3 rgb, vec2 xy)
{
  ...
  // Tint and vignette...
  rgb *= vec3(.5, .5, .4) + 0.5*pow(40.0*xy.x*xy.y*(1.0-xy.x)*(1.0-xy.y), 0.2 );  
  // Noise...
  rgb = clamp(rgb-Hash(rgb.rb+xy*iGlobalTime)*.075, 0.0, 1.0);
...
void main(void)
{
  ...
  float distance;
  float type;
  if( !Scene(cameraPos, direction, distance, type) )
  {
  ...
  else
  {
    ...
    vec3 pos = cameraPos + distance * direction;
    ...
    col = TerrainColour(pos, direction, nor, distance, type);
  }
