// from https://www.shadertoy.com/view/MsfSW4
#define STEPS 128
...
{	
  ...
  light += oog;
	...
	oog.y = light.y = -sin(oog.z);
	oog.x = light.x = -sin(oog.z);
	
	Ray ray = lookAt(oog, light);

  if (!setup_ray(oog, dir, ray.origin, ray.direction)) return;  // boxplorify

	ray.direction *= rotX(-cos(oog.z+0.4));
	ray.direction *= rotY(cos(oog.z));
	
	Object o = march(ray);
	
	float res = 5.0;
	vec3 col = texture2D(iChannel0,o.position.xz*res).rgb;
  ...
  write_pixel(dir, o.distance, col);  // boxplorify
}
