...
/* Controls: numbers are the keys. x <=> left/right arrows. y <=> up/down arrows.
	  ...
	  KIFS Mengers:
		...
		[7].x size of bounding box
		[8].y x component of tiling
		[8].x y component of tiling
...
//3D coordinates infinite tiling
vec3 Tile3D(vec3 p, vec3 a){
	p+=a;
	p-=4.0*a*floor(p/(4.0*a));
	p-=2.0*max(p-2.0*a,0.0);
	p-=a;
	return p;
}
...
//1D coordinates infinite tiling
...
#define Siz par[5].x//0.25//
	z0*=1.0/Siz;
	z0.xy=Tile2D(z0.xy,vec2(par[8].y,par[8].x));//2D infinitely tiled Menger 
	int i;
	vec3 kz=abs(z0);
	float r=max(kz.x,max(kz.y,kz.z));
	for (i=0;i<int(MenIters) && r<10.0;i++){
		z0=abs(z0);
	...
	return (r-par[7].x)*pow(MenSCALE,-float(i))*Siz;
}
