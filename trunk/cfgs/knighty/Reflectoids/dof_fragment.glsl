...
// box
const float gb_offset[13] = {-11.5, -9.5, -7.5, -5.5, -3.5, -1.5, 0, 1.5, 3.5, 5.5, 7.5, 9.5, 11.5};
...
#define S(x) pow((x), vec4(1/GAMMA,1/GAMMA,1/GAMMA,1.0))
#else
#define P(x) (x)
#define S(x) (x)
#endif
...
vec4 mixDimension(vec2 coord) {
  ...
  if (iBlurLevel == 0) {
    // (1, 0) or (0, 1) : square
    xory = vec2(iXorY, 1. - iXorY) * texRez * centerPixel.a;
  } else {
    // (s2, s2) or (s2, -s2) : square rotate 45 degrees
    xory = vec2(SQRT2, 2.*(.5 - iXorY) * SQRT2) * texRez * centerPixel.a;
  }

  for (int i = 0; i < 13; ++i) {
    vec2 sampleCoord = coord + xory * gb_offset[i];
    ...
    float weight = (sampleDepth < centerDepth) ?
        samplePixel.a * bleedingMult : 1.0f;
