...
void main( void ) {
	...
	float f0 = .5; //fract(time*0.05)+1e-5;
	float f = fract(f0*2.);
	float fs = sign(f-.5);
	float fs0 = sign(f0-.5);

#if 0
	vec3 ray = normalize(vec3(vec2(gl_FragCoord.x / resolution.x - 0.5, (gl_FragCoord.y - resolution.y * 0.5) / resolution.x), 0.5));
	...
	ray *= mat3(uc,-ts*us,-tc*us,0,tc,-ts,us,ts*uc,tc*uc);
	//ray *= vec3(sign(f-.5),sign(f-.5),1.);
	ray.z *= fs;
	...
	// As last step position & rayection are rotated as camera would otherwise fly through an edge instead of a face
	float x = ac*bs;
	float z = ac*bc*cs;
	vec3 pos = vec3(x*a+z,as*e,-x+a*z)/(ac*bc*cc*e);
	//ray = fs;
	ray = normalize(vec3(ray.x*ac*cc-ac*bs*ray.z*cs,-as*ray.z*cs-ray.x*as*bs*cc+ray.y*bc*cc,ac*bc*ray.z)*mat3(a,0,1, 0,e,0, -1.,0,a));
#endif
  ...
  if (!setup_ray(eye, dir, pos, ray)) {
	...
	for (int i = 0; i < 10; i++) {
		...
		float kDist = kleinDist(pos, ray);	// distance to enclosing polyhedron
		
		pos += ray*min(sDist,kDist);	// compute actual distance (as we're in the klein metric we can't simply do length(a-b) - we have to use
		vec4 hpos2 = kleinToHyper(pos);
		cremain *= exp(-.3*hyperdist(hpos, hpos2)); //... and simulate fog
		hpos = hpos2;
			
		if (sDist < kDist) {
			ray = reflect(ray, -normalize(pos));		// reflect off sphere (as it's around the origin a simple reflection will do it)
			...
			ray = hreflect(pos, ray,fa,fb,fc);	// reflect off polyhedron (advanced math stuff) - simulates propagation into "next" polyhedron
		}
